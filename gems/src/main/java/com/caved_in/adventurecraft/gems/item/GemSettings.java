...
public class GemSettings {
    ...
    private List<ChancedItemEffect> effects = new ArrayList<>();
    ...
    public GemSettings addEffect(ChancedItemEffect effect) {
        effects.add(effect);
        return this;
    }
    ...
    public boolean hasSuffixForEffect(ItemEffect effect) {
        for (GemSuffixSettings suffixData : suffixSettings) {
            if (!suffixData.hasEffect()) {
                continue;
            }

            if (suffixData.getEffect().name().equalsIgnoreCase(effect.name())) {
                return true;
            }
        }
        return false;
    ...
    public ChancedItemEffect getRandomItemEffect() {
        if (effects.isEmpty()) {
            return null;
        }

        return ListUtils.getRandom(effects);
    }
...
//            Chat.debug("SETTINGS FOR SUFFIX " + enchantment.getName() + " @ lvl " + level + " is null!");
    ...
    public boolean hasSuffixFor(ItemEffect effect) {
        for (GemSuffixSettings settings : suffixSettings) {
            if (!settings.hasEffect()) {
                continue;
            }

            if (settings.getEffect().name().equalsIgnoreCase(effect.name())) {
                return true;
            }
        }
    ...
    public GemSuffixSettings getSuffixSettings(ItemEffect effect) {
        for(GemSuffixSettings settings : suffixSettings) {
            if (!settings.hasEffect()) {
                continue;
            }
    ...
    public GemSettings defaultItemEffects() {
        for(ChancedItemEffect effect : ChancedItemEffect.DEFAULT_EFFECTS) {
            effects.add(effect);
        }
        return this;
    }
